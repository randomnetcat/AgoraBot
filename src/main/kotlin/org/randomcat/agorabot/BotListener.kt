...
class BotListener(private val parser: CommandParser, private val invoker: CommandInvoker) {
    @SubscribeEvent
    fun onMessage(event: MessageReceivedEvent) {
        return when (val parseResult = parser.parse(event)) {
            is CommandParseResult.Invocation -> invoker.invokeCommand(event, parseResult.invocation)
