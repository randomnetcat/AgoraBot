...
inline fun <reified T> StringState.get(key: String): T? {
    return getString(key)?.let { Json.decodeFromString<T>(it) }
}

inline fun <reified T> StringState.getMany(keys: List<String>): List<T?> {
    return getStrings(keys).map { str -> str?.let { Json.decodeFromString<T>(it) } }
}

inline fun <reified T> StringState.set(key: String, value: T) {
    setString(key, Json.encodeToString<T>(value))
}

inline fun <reified T> StringState.setMany(keys: List<String>, values: List<T>) {
    setStrings(keys, values.map { Json.encodeToString<T>(it) })
}

inline fun <reified T> StringState.update(key: String, crossinline mapper: (old: T?) -> T) {
    updateString(key) { oldString ->
        Json.encodeToString<T>(mapper(oldString?.let { Json.decodeFromString<T>(it) }))
    }
}

inline fun <reified T> StringState.updateMany(keys: List<String>, crossinline mapper: (old: List<T?>) -> List<T>) {
